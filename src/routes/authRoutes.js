...
router.post('/send-verification-code', async (req, res, next) => {
  try {
    ...
    if (userExists && userExists.lastVerificationCodeSentAt) {
      const timeSinceLastCode = (Date.now() - userExists.lastVerificationCodeSentAt) / 1000;
      if (timeSinceLastCode < 60) {
        return res
          .status(400)
          .json({ message: `Please wait ${Math.ceil(60 - timeSinceLastCode)} seconds before requesting another code` });
      }
    }
    ...
    const sent = await sendVerificationEmail(email, code, 'User');
    if (!sent) {
...
router.post('/register', validateRegister, async (req, res, next) => {
  try {
    ...
    let user = await User.findOne({ email }).select('+emailVerificationCode +emailVerificationExpires');
...
router.post('/login', validateLogin, async (req, res, next) => {
  try {
    ...
    if (!user.emailVerified) {
      return res
        .status(403)
        .json({ message: 'Please verify your email first. Check your inbox for the verification code.' });
    }
