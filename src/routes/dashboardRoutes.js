...
router.get('/summary', protect, async (req, res, next) => {
  try {
    ...
    if (role === 'admin') {
      const [totalUsers, totalProjects, totalTasks, completedTasks, pendingTasks, members, pms] =
        await Promise.all([
          User.countDocuments(),
      ...
      return res.json({
        success: true,
        data: { totalUsers, totalProjects, totalTasks, completedTasks, pendingTasks, members, projectManagers: pms },
      });
    }
    ...
    if (role === 'project_manager') {
      ...
      const [totalProjects, totalTasks, completedTasks, pendingTasks, inProgressTasks] =
        await Promise.all([
          ...
          Task.countDocuments({ projectId: { $in: projectIds }, status: 'To Do' }),
          Task.countDocuments({ projectId: { $in: projectIds }, status: 'In Progress' }),
        ]);

      return res.json({
        success: true,
        data: { totalProjects, totalTasks, completedTasks, pendingTasks, inProgressTasks },
      });
    }
    ...
    return res.json({
      success: true,
      data: { totalTasks, completedTasks, pendingTasks, inProgressTasks, projects: myProjects },
    });
  } catch (error) {
    next(error);
  }
